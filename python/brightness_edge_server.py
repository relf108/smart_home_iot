...
while True:
    ...
    # If there is motion, allow sending of softpot information to backend
    if value == 'true':
        # read softpot value from Arduino
        brightness = arduino_connection.analog_read(11)

        # send temperature value to backend
        request = requests.post('/setBrightness/' + brightness)