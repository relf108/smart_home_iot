/* Credit: https://github.com/lekum/pyduino - but modified for extra functionality
 * 
...
void digital_play(int pin_number, int digital_value) {
    /*
     * Plays a tone through a speaker using digital_value as the frequency with a moderate release time
     * Due to a limitation of the tone() function, frequency must be above 31Hz
     */ 
  tone(pin_number, digital_value, 500);
}
...
void loop() {
    ...
    if (Serial.available() > 0) {
        ...
        switch (operation){
            ...
            case 'P': // Play operation, e.g. PD11:851
                if (mode == 'D'){
                    digital_play(pin_number, value_to_write);
                } else {
                    break; // Unexpected mode
                }
                break;
